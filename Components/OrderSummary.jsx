export default function OrderSummary({ allOrder }) {
  const pending = allOrder.filter((item) => item.status === false);
  const delivered = allOrder.filter((item) => item.status === true);
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-cardbg rounded-lg p-4 relative overflow-hidden">
          ...
          <div className="bg-red-900 bg-opacity-50 text-red-200 text-xs font-medium px-3 py-1 rounded-full inline-block">
            Pending
          </div>
        </div>
